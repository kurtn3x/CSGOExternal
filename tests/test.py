            ...
            try:
                ...
                if g_aimbot:
                    ...
                    if not _target.is_valid() and not get_best_target(view_angle, self):
                        continue
                    try:
                        x, y = aim_at_target(fl_sensitivity, view_angle, get_target_angle(self, _target, _target_bone))
                        mem.write_float(nv.dwClientState + nv.dwViewAngles, x)
                        mem.write_float(nv.dwClientState + nv.dwViewAngles + 0x4, y)
                    except:
                        pass
