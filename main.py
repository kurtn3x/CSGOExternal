...
from funcs.aimbot import LocalPlayer, TargetPlayer, CalcDistance
...
class AimbotThread(QObject):
    ...
    def run(self):
        while True:
            ...
            print(local_player.Origin.x, local_player.Origin.y, local_player.Origin.z)
            ...
            if closestDistanceIndex == -1:
                ...
                print(CalcDistance(local_player.ViewOffset, closestEnemy.BonePos))
                print(closestEnemy.Aimspot)
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.worker = WindowThread()
        self.worker.start()
        self.worker.update_progress.connect(self.open_close)
        self.mainwindow_ui.glowButton.clicked.connect(self.start_glow)
        self.mainwindow_ui.aimbotButton.clicked.connect(self.start_aimbot)
        self.glow = GlowThread()
        self.aimbot_obj = AimbotThread()
        self.aimbot = QThread()
        self.aimbot_obj.moveToThread(self.aimbot)
        self.aimbot.started.connect(self.aimbot_obj.run)
        self.glow_enabled = False
        self.aimbot_enabled = False
        self.aimbot_obj.update_localpos.connect(self.update_localpos)
        self.aimbot_obj.update_targetpos.connect(self.update_targetpos)
        self.aimbot_obj.update_targetpos2.connect(self.update_targetpos2)
        self.aimbot_obj.update_viewangles.connect(self.update_viewangles)
